...
* Built-in <b>API access</b> to serve <b>JSON</b> content to external webpages, which can serve that content with AJAX.  **API token keys are registered to individual users, and calls have pagination limits. 
**API was built following basic TDD, via <tt>rspec-rails</t>.
